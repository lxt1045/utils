...
func Test_schemaDiff(t *testing.T) {
	fromURL := []string{"mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/dji1"}
	toURL := []string{"file://e:/test/atlas/test.sql"}
	schemas, exclude := []string{}, []string{}
	formatTo := "{{ sql . \"  \" }}"
	devURL := "mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/atlas_dev"
...
func Test_schemaInspectRun(t *testing.T) {
	fromURL := []string{"mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/dji88"}
	// toURL := []string{"file://e:/test/atlas/test.sql"}
	schemas, exclude := []string{}, []string{}
	formatTo := "{{ sql . \"  \" }}"
	devURL := "mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/atlas_dev"
...
// 1. 基线文件, initial 只是名字
atlas migrate diff initial --dir "file://e:/test/atlas/migrations?format=golang-migrate" --to "file://e:/test/atlas/test.sql" --dev-url "mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/atlas_dev" --format '{{ sql . \"  \" }}'

// 2. 如果数据库已存在，你需要告诉 Atlas 不要应用这个基线，因为它已经存在了
// 结果存于 url 指向的DB的 atlas_schema_revisions 表中
atlas migrate set 20260807064528 --dir "file://e:/test/atlas/migrations?format=golang-migrate"  --url "mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/dji1"

// 3. 修改 test.sql 后, 会与 migrations/ 目录下已有的所有迁移文件进行对比，生成一个新的版本化 SQL 文件
atlas migrate diff test_dev_name --dir "file://e:/test/atlas/migrations?format=golang-migrate" --to "file://e:/test/atlas/test.sql" --dev-url "mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/atlas_dev" --format '{{ sql . \"  \" }}'
...
atlas migrate apply --dir "file://e:/test/atlas/migrations?format=golang-migrate" - --url ""mysql://root:BdtEnxxTnoN1luUR@10.1.1.121:3306/dji88"
*/
