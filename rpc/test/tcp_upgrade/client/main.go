...
import (
	...
	"github.com/lxt1045/utils/rpc/test/tcp_upgrade/filesystem"
	"github.com/lxt1045/utils/rpc/test/tcp_upgrade/pb"
...
func main() {
	...
	}
	{
		...
		buf := []byte("1111111111111111")
		_, err = upgrade.Write(buf)
		if err != nil {
			log.Ctx(ctx).Error().Caller().Err(err).Send()
			return
		}
		...
		go func() {
			...
			for range 2 {
				...
				log.Ctx(ctx).Info().Caller().Str("upgrade read", string(buf[:n])).Send()
			}
		}()
	}
